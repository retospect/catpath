...
def caption(cfg: Config, results) -> str:
    ...
    return (
        ...
        f"band / Ea+- = 1 s.d.  |  autocatpath v{__version__}"
    )
...
layers fixed, {cfg.slab.vacuum} A vacuum) were explored with the *autocatpath* pipeline
...
def png_metadata(cfg: Config, results) -> dict[str, str]:
    ...
    return {
        ...
        "Software": f"autocatpath v{__version__}",
