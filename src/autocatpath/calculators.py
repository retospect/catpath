...
* ``emt``      - ASE Effective Medium Theory.  Pure-numpy, installs anywhere, no
                 ...
                 C N O H only.
* ``mace``     - MACE-MP-0 universal potential (``pip install autocatpath[mace]``).
* ``chgnet``   - CHGNet universal potential (``pip install autocatpath[chgnet]``).
* ``fairchem`` - Meta FAIRChem / UMA, purpose-built for adsorbates on metals via
                 the OC20 task (``pip install autocatpath[fairchem]``; UMA weights are
                 license-gated -- needs a Hugging Face login).
* ``grace``    - GRACE foundation models (``pip install autocatpath[grace]``).
...
def resolve_backend(backend: str) -> str:
    ...
    for cand in AUTO_ORDER:
        if _installed(cand):
            return cand
    hint = " ".join(f"autocatpath[{_EXTRA[b]}]" for b in AUTO_ORDER)
    raise RuntimeError(
...
def _load(backend: str, cfg: MLIPConfig):
    ...
    try:
        ...
        raise RuntimeError(
            f"backend '{backend}' is not installed: pip install autocatpath[{extra}] "
            f"(import failed: {e})"
        ) from e
