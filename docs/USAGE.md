...
autocatpath seed <cfg> --seed 0 --out p0.json # one seed → partial JSON (fan-out unit)
autocatpath aggregate <cfg> --partials p*.json # combine partials → outputs
autocatpath sweep <cfg> --elements Pd,Pt,Cu   # same network across surfaces → multi-row map
autocatpath multi <cfg>                       # several substrates (config `substrates:`) → union map
autocatpath states <cfg> --out s_mace.json    # relax states only (no NEB) → per-model JSON
autocatpath barriers <cfg> --out b_mace.json  # NEB for every step → per-model barrier JSON
autocatpath compare --states s_*.json --out cmp.png   # merge → box plot per state, dots per model
autocatpath compare --states b_*.json --out bars.png  # barrier JSONs → Ea box plot, rate-limiting ringed
autocatpath compare --states b_*.json --heights s_*.json --out ts.png  # TS heights, highest point starred
autocatpath <cfg>                            # shorthand for `run`
autocatpath run --substrate NO --target NH3 --element Pd --network auto   # no config file
...
```bash
.venv-chgnet/bin/autocatpath   states cmp.yaml --backend chgnet   --device cuda --out s_chgnet.json
.venv-fairchem/bin/autocatpath states cmp.yaml --backend fairchem --device cpu  --out s_fairchem.json
uv run autocatpath           states cmp.yaml --backend mace     --device cuda --out s_mace.json
uv run autocatpath compare --states s_*.json --out compare.png
```
