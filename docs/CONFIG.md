# Configuration reference — how you tell autocatpath what to do

A run is **one YAML file**: `autocatpath run my.yaml`. Every field below, with the
...
| `mace` | MACE-MP-0 universal | `pip install autocatpath[mace]` | GPU; solid general default. |
| `chgnet` | CHGNet universal | `pip install autocatpath[chgnet]` | CPU-friendly. |
| `fairchem` | Meta FAIRChem / UMA | `pip install autocatpath[fairchem]` | Purpose-built for adsorbates-on-metals (OC20 task); UMA weights **license-gated** (HF login). |
| `grace` | GRACE foundation models | `pip install autocatpath[grace]` | TensorFlow-based. |
...
Two ways to get a network. **Curated** templates in `src/autocatpath/network.py` are
...
Set `network: auto` and autocatpath **generates** the intermediates from
`substrate` → `target` (see `src/autocatpath/explore.py`). It applies three
...
## §Substrates — multi-substrate via `autocatpath multi` ✅
...
`autocatpath multi my.yaml`. Each entry gets its own full run + per-run artifacts,
...
- **`autocatpath sweep --elements Pd,Pt,Cu,Ni`** — same network across surfaces
...
absent, autocatpath **prints a warning, records it in `results.json`, and falls back
...
| Substrates (how many) | `substrates: [...]` + `autocatpath multi` | ✅ multi-substrate |
